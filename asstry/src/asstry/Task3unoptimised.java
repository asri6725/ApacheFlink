...
public class Task3unoptimised {
	...
	public static <R> void main(String[] args) throws Exception {
	    ...
	    DataSet<Tuple2<String, String>> airports =
	  	...
	  	DataSet<Tuple3<String, String, Integer>> cairportss = airports.flatMap(new airportmapper()); 
	    ...
	    //res.writeToSink(new CsvTableSink(output_filepath, "\t", 1, WriteMode.OVERWRITE)); --NOT NEEDED
	    ...
	    /*One way i think to solve this task is the O(n^2) one. 
	     * Iterate over the dataset resultds once (i)
	     * 		Iterate again over resultds (j)
	     * 			if j.string1 == i.string1
	     * 				collect j.string1, [j.string2, i.string2] 
	     * 
	     * but the problem was we cannot iterate over datasets like this.
	     * but apparently, we can "broadcast" a dataset for all other datasets to use
	     * 
	     * so the resultds will be constant and accessible by resultds2 (=resultds)
	     * map func iterates over each tuple in a dataset. So in that map, we can iterate over the broadcasted set
	     * and perform something similar to ^^ algo
	     */
	    ...
	    DataSet<Tuple2<String, List<String>>> r = resultds3.distinct(0); // for distinct fields

	    Table write = tableEnv.fromDataSet(r, "airline, models");
	    
	    write.writeToSink(new CsvTableSink(output_filepath, "\t", 1, WriteMode.OVERWRITE));
		
	    //r.writeAsText(output_filepath, WriteMode.OVERWRITE);
	    
	    // execute the FLink job
	    env.execute("Executing task 3 unopt");
	  }
